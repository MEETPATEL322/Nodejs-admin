...
import {fetchNewsList,fetchNewsListLoadMore} from "../redux/action/newsAction";
...
const News: FC<Props> = ({
  ...
  useEffect(() => {
    ...
    if (page) {
    fetchNewsList(page); // eslint-disable-next-line react-hooks/exhaustive-deps
    }
  }, []);

  const loadMore = () => {
    // setPage(page + 1);
    fetchNewsListLoadMore(page + 1);
  };

  console.log("News data=========", listData);
  return (
    <>
      <MainSlider />
              ...
              listData.map((item: any, index: number) => {
                return (
                  ...
                  >
                    ...
                    <ListItem mainId={item._id} poster={`${process.env.REACT_APP_API_URL}/${item.newsimage}`} 
                    name={item.newstitle} 
...
const mapStateToProps = (state: any) => {
  return {
    listData: state.newsReducer.listData,
    totalPage: state.newsReducer.totalPage,
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    fetchNewsList: (page: number) => dispatch(fetchNewsList(page)),
    fetchNewsListLoadMore: (page: number) => dispatch(fetchNewsListLoadMore(page)),
  };
};
